...
contract ShareWrapper {
    ...
    function withdraw(uint256 amount) public virtual {
        ...
        _balances[msg.sender] = groveUserEsteem.sub(amount);
...
contract Staking is ShareWrapper, ContractGuard, Operator {
    ...
    function latestSnapshotIndex() public view returns (uint256) {
        return groveHistory.length.sub(1);
    }
    ...
    function canWithdraw(address groveUser) external view returns (bool) {
        return grovers[groveUser].epochTimerStart.add(withdrawLockupEpochs) <= treasury.epoch();
    }

    function canClaimReward(address groveUser) external view returns (bool) {
        return grovers[groveUser].epochTimerStart.add(rewardLockupEpochs) <= treasury.epoch();
    }
    ...
    function earned(address groveUser) public view returns (uint256) {
        ...
        return balanceOf(groveUser).mul(latestRPS.sub(storedRPS)).div(1e18).add(grovers[groveUser].rewardEarned);
    }
    ...
    function withdraw(uint256 amount) public override onlyOneBlock groveUserExists updateReward(msg.sender) {
        require(amount > 0, "Grove: Cannot withdraw 0");
        require(grovers[msg.sender].epochTimerStart.add(withdrawLockupEpochs) <= treasury.epoch(), "Grove: still in withdraw lockup");
        claimReward();
    ...
    function claimReward() public updateReward(msg.sender) {
        ...
        if (reward > 0) {
            require(grovers[msg.sender].epochTimerStart.add(rewardLockupEpochs) <= treasury.epoch(), "Grove: still in reward lockup");
