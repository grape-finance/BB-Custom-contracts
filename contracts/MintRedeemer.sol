...
contract MintRedeemer is Ownable2Step, ReentrancyGuard, Pausable, IPriceProvider {
    ...
    function mintEsteemWithPLS(uint256 deadline) external payable nonReentrant whenNotPaused {
        ...
        _depositToStronghold(WPLS, msg.value);
    ...
    function _calculateRedeemAmounts(uint256 esteemAmount, address favorToken) internal view returns (uint256 userReceives) {
        ...
        uint256 rate = esteemRate();
        uint256 esteemToFavor = (esteemAmount * rate) / favorPrice;
    ...
    // Get current esteem rate base on block.timestamp for an increase per 24hrs
    function esteemRate() public view returns (uint256) {
        if (block.timestamp < startTime) return initialEsteemRate;
    ...
    function setInitialEsteemRate(uint256 _rate) external onlyOwner {
