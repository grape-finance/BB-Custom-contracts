...
library PulseXLibrary {
    ...
    function getReserves(
        ...
        (uint reserve0, uint reserve1, ) = IPulseXPair(
    ...
    function quote(
        ...
        require(
            ...
            "PulseXLibrary: INSUFFICIENT_LIQUIDITY"
        );
...
contract PulseXRouter02 is IPulseXRouter02 {
    ...
    function setFavorToken(address _favorToken) external {
        favorToken = _favorToken;
    }
        ...
        returns (uint[] memory amounts)
    {
        ...
        require(
            ...
            "PulseXRouter: EXCESSIVE_INPUT_AMOUNT"
        );
        TransferHelper.safeTransferFrom(
            path[0],
            msg.sender,
    ...
    function _swapSupportingFeeOnTransferTokens(
        ...
        for (uint i; i < path.length - 1; i++) {
            ...
            IPulseXPair pair = IPulseXPair(
                PulseXLibrary.pairFor(factory, input, output)
            );
            uint amountInput;
            uint amountOutput;
            {
                ...
                amountOutput = PulseXLibrary.getAmountOut(
                    amountInput,
                    reserveInput,
                    reserveOutput
                );
